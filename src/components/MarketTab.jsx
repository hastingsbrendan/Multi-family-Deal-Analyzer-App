...
function RateCompare({fredRate,dealRate}){
  ...
  return(<div style={{marginTop:12,padding:'10px 14px',borderRadius:10,background:isHigher?'#f59e0b18':isLower?'#10b98115':'var(--card)',border:`1px solid ${isHigher?'#f59e0b55':isLower?'#10b98144':'var(--border)'}`}}><div style={{fontSize:12,fontWeight:700,color:'var(--text)',marginBottom:4}}>{isHigher?'⚠️':isLower?'✅':'ℹ️'} Your Deal Rate vs. Market</div><div style={{fontSize:12,color:'var(--muted)'}}>You modeled <strong style={{color:'var(--text)'}}>{dealRate.toFixed(2)}%</strong> vs current 30-yr avg of <strong style={{color:'var(--accent)'}}>{fredRate.toFixed(2)}%</strong> — {isHigher?`${absDiff}% above market (conservative ✓)`:isLower?`${absDiff}% below market — review your rate assumption`:'roughly in line with market rates'}</div></div>);
}
...
function MarketTab({deal}){
  ...
  const [saleFilter,setSaleFilter]=useState('mf'); // 'mf' | 'all'
  ...
  const fetchAll=useCallback(async(zipCode)=>{
    ...
    try{
      ...
      if(censusRes.ok){
        ...
        if(raw.length>=2){const h=raw[0];const v=raw[1];const obj={};h.forEach((k,i)=>{obj[k]=v[i];});setCensusData(obj);}
      }
      setLastZip(zipCode);
  ...
  // active sale data: MF-filtered or all-types, based on toggle (fall back to all if no MF data)
  ...
  const rateDelta=currentRate&&rateLastYear?(currentRate-rateLastYear).toFixed(2):null;
      ...
      <div style={gridStyle}>
        ...
        {rd&&(
          <Section>
            ...
            {bedroomRents.length>0&&(<div style={{marginBottom:16}}><div style={{fontSize:11,fontWeight:700,color:'var(--muted)',textTransform:'uppercase',letterSpacing:'0.08em',marginBottom:8}}>Avg Rent by Bedrooms</div>{bedroomRents.map(b=>(<div key={b.beds} style={{display:'flex',justifyContent:'space-between',alignItems:'center',padding:'7px 0',borderBottom:'1px solid var(--border)'}}><div style={{fontSize:12,color:'var(--muted)',fontWeight:600}}>{b.beds}</div><div style={{display:'flex',gap:16,alignItems:'center'}}>{b.count>0&&<div style={{fontSize:10,color:'var(--muted)'}}>{b.count} listings</div>}<div style={{fontSize:13,fontWeight:800,color:'var(--accent)'}}>{b.avg?FMT_USD(b.avg):'—'}</div></div></div>))}</div>)}
        ...
        {/* SALE MARKET — toggle between MF-only and all property types */}
        {sd&&(
          <Section>
            ...
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',marginBottom:16}}>
              <div>
                ...
                <div style={{fontSize:11,color:'var(--muted)',marginTop:2}}>
                  {sdIsMF?`${sdActive.totalListings||0} multi-family listings · Rentcast`:`${sd.totalListings||0} listings (all types) · Rentcast`}
                </div>
              </div>
              {/* MF / All toggle pill */}
              {sdMF&&(
                <div style={{display:'flex',background:'var(--bg2)',borderRadius:100,padding:2,gap:0,border:'1px solid var(--border)',flexShrink:0}}>
                  {[['mf','Multi-Family'],['all','All Types']].map(([val,label])=>(
                    <button key={val} onClick={()=>setSaleFilter(val)} style={{
                      padding:'4px 11px',fontSize:11,fontWeight:700,borderRadius:100,border:'none',cursor:'pointer',
                      fontFamily:'inherit',transition:'all 0.15s',whiteSpace:'nowrap',
                      background:saleFilter===val?'var(--accent)':'transparent',
                      color:saleFilter===val?'#fff':'var(--muted)'
                    }}>{label}</button>
                  ))}
                </div>
              )}
            ...
            {!sdMF&&saleFilter==='mf'&&(
              <div style={{fontSize:11,color:'var(--accent2)',fontStyle:'italic',marginBottom:10,padding:'6px 8px',background:'rgba(217,119,6,0.07)',borderRadius:6}}>
                ⚠ No multi-family listings in this ZIP — showing all property types.
              </div>
            )}
        ...
        {rd?.averageRent>0&&sd?.medianPrice>0&&(
          <Section>
            ...
            {deal?.assumptions?.purchasePrice>0&&(()=>{
              ...
              return(<><div style={{height:1,background:'var(--border)',margin:'10px 0'}}/><div style={{fontSize:11,fontWeight:700,color:'var(--muted)',textTransform:'uppercase',letterSpacing:'0.06em',marginBottom:8}}>Your Deal vs. Market</div><StatRow label="Your GRM" value={`${grm.toFixed(1)}x`} sub={`vs ${mktGrm.toFixed(1)}x market`} accent={grm<mktGrm}/><StatRow label="Your Gross Yield" value={`${yield_.toFixed(1)}%`} sub={`vs ${((rd.averageRent*12)/sd.medianPrice*100).toFixed(1)}% market`}/></>);
            })()}
          </Section>
        )}
      </div>
        ...
        {currentRate&&!fredLoading&&(
          <>
            <div style={{display:'flex',gap:12,flexWrap:'wrap',marginBottom:16}}>
              <MetricCard label="30-Yr Fixed Rate" value={`${currentRate.toFixed(2)}%`} sub={rateDelta?`${+rateDelta>0?'▲':'▼'} ${Math.abs(+rateDelta).toFixed(2)}% vs 1yr ago`:'Current weekly avg'} highlight/>
            ...
            {fredChartData.length>3&&(
              <div style={{marginTop:16}}>
                <div style={{fontSize:11,fontWeight:700,color:'var(--muted)',textTransform:'uppercase',letterSpacing:'0.06em',marginBottom:10}}>6-Month Rate Trend</div>
                <ResponsiveContainer width="100%" height={160}><LineChart data={fredChartData} margin={{top:4,right:16,left:0,bottom:4}}><CartesianGrid strokeDasharray="3 3" stroke="var(--border)"/><XAxis dataKey="date" tick={{fontSize:10,fill:'var(--muted)'}} tickFormatter={v=>v.slice(5)}/><YAxis tick={{fontSize:10,fill:'var(--muted)'}} tickFormatter={v=>`${v.toFixed(1)}%`} width={44} domain={['auto','auto']}/><Tooltip content={<ChartTooltip formatter={v=>`${v.toFixed(2)}%`}/>}/>{dealRate&&<Line type="monotone" dataKey={()=>dealRate} stroke="var(--accent2)" strokeWidth={1.5} dot={false} name="Your Rate" strokeDasharray="5 3"/>}<Line type="monotone" dataKey="rate" stroke="var(--accent)" strokeWidth={2} dot={false} name="30-Yr Avg"/></LineChart></ResponsiveContainer>
                <div style={{display:'flex',gap:16,justifyContent:'center',marginTop:8}}><div style={{display:'flex',alignItems:'center',gap:6,fontSize:11,color:'var(--muted)'}}><div style={{width:24,height:2,background:'var(--accent)',borderRadius:2}}/> 30-Yr Avg</div>{dealRate&&<div style={{display:'flex',alignItems:'center',gap:6,fontSize:11,color:'var(--muted)'}}><div style={{width:24,height:2,background:'var(--accent2)',borderRadius:2}}/> Your Rate</div>}</div>
              </div>
            )}
          </>
        )}
